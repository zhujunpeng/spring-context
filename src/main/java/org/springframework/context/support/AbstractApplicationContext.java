...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// ApplicationContext是对BeanFactory的扩展，所以包括BeanFactory所有的功能，还添加了大量的功能
			...
			try {
				...
				// 初始化应用消息广播器，并放入applicationEventMulticaster bean 中
				...
				// 完成刷新过程，通知生命周期处理器刷新过程，同时发出ContextRefreshEvent事件
				finishRefresh();
			}
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 这个就是BeanFactory进行初始化，进行xml文件读取，并将得到的BeanFactory记录在当前实体属性中
		refreshBeanFactory();
		// 返回当前实例的属性
